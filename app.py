import sys
from flask import Flask, get_flashed_messages, render_template, request, redirect, url_for, flash, session, send_file, make_response, jsonify
...
# Try to import WeasyPrint, but handle the case when it's not available
try:
    from weasyprint import HTML
    weasyprint_available = True
except OSError as e:
    logging.error(f"WeasyPrint could not be imported: {e}")
    logging.error("To fix this issue on Windows, install GTK+ from: https://github.com/tschoonj/GTK-for-Windows-Runtime-Environment-Installer/releases")
    logging.error("After installing GTK+, restart your computer and try again.")
...
def create_sample():
    ...
    users = [
        User(full_name='احمد الوكيل', email='ahmed.elwakeel@system.com', roles='["general_department_member"]', governorate='الإسكندرية', active=True),
        User(full_name='محمد علي', email='mohamed.ali@system.com', roles='["general_admin"]', governorate='الجيزة', active=True),
        User(full_name='مريم حسن', email='mariam.hassan@system.com', roles='["central_admin"]', governorate='الدقهلية', active=True),
...
def generate_pdf(data=None, data_type='committee'):
    ...
    if not data:
        ...
        if not weasyprint_available:
            flash("لا يمكن إنشاء ملف PDF. يرجى تثبيت GTK+ من الرابط المذكور في سجل الخطأ.", "error")
            return None
        HTML(string=html_content).write_pdf(buffer)
        buffer.seek(0)
        return buffer

# دالة مساعدة لإنشاء استجابة PDF
def create_pdf_response(data=None, data_type='committee', filename='document.pdf'):
    pdf_buffer = generate_pdf(data, data_type)
    ...
    try:
        ...
        flash("حدث خطأ أثناء إنشاء ملف PDF.", "error")
        return None
...
def signup():
    if request.method == 'POST':
        ...
        governorate = request.form.get('governorate', 'غير محدد')  # Make governorate optional with default value
...
def change_password():
    ...
    if len(new_password) < 8:
        flash('يجب أن تحتوي كلمة المرور على 8 أحرف على الأقل', 'error')
        return redirect(url_for('profile'))
    
    # التحقق من وجود حرف كبير ورقم ورمز خاص
    if not (any(c.isupper() for c in new_password) and 
        ...
        flash('يجب أن تحتوي كلمة المرور على حرف كبير ورقم ورمز خاص على الأقل', 'error')
        return redirect(url_for('profile'))
    ...
    try:
        ...
        logging.info(f"تم تغيير كلمة المرور للمستخدم {user.full_name}")
    except Exception as e:
        db.session.rollback()
        logging.error(f"خطأ أثناء تغيير كلمة المرور: {e}")
        flash('حدث خطأ أثناء تغيير كلمة المرور، حاول مرة أخرى', 'error')
    ...
    try:
        ...
        if decision_type == 'appointment':
            ...
            if latest_appointment:
                ...
                response = create_pdf_response(appointment_data, data_type='appointment', filename=f'appointment_{latest_appointment.decision_number}.pdf')
                if response is None:
                    return redirect(url_for('dashboard'))
                return response
            else:
                response = create_pdf_response(filename='no_data.pdf')
                if response is None:
                    return redirect(url_for('dashboard'))
                return response
